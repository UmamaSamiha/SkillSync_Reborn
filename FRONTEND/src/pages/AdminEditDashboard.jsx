...
const EMAILJS_PUBLIC_KEY  = 'FWtZeCxoli26Rk021';

async function sendCongratulationsEmail(studentName, studentEmail, topicName, onSuccess) {
  try {
    await emailjs.send(
      EMAILJS_SERVICE_ID,
      EMAILJS_TEMPLATE_ID,
    ...
    toast.success(
      `🎉 Congratulations email sent to ${studentName} for unlocking "${topicName}"!`,
      { duration: 6000 }
    );
    if (onSuccess) onSuccess({ studentName, studentEmail, topicName, sentAt: new Date().toISOString() });
  } catch (err) {
    console.error('EmailJS error:', err);
    toast.error('Score saved but email failed to send.');
  }
}
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        ...
        <span style={{
          ...
          background: 'rgba(34,197,94,.15)', color: 'var(--color-green)',
        }}>
          {logs.length} sent
        </span>
      </div>
          ...
          <div key={i} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--color-muted)', marginTop: 2 }}>
                Unlocked topic: <strong style={{ color: 'var(--color-green)' }}>{log.topicName}</strong>
              </div>
            </div>
...
function ScorePanel({ submission, onScored, onEmailSent }) {
  ...
  const handleScore = async () => {
    ...
    if (res.data.unlocked || scoreNum >= 75) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--color-muted)', marginBottom: 28 }}>
        Monitor student submissions, detect paste events, compare versions, and assign scores.
      </p>

      <EmailLogPanel logs={emailLogs} />
